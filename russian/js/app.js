...
function openModal({ title, text, primaryText = "Continuar", secondaryText = null, tertiaryText = null, onPrimary = null, onSecondary = null, onTertiary = null }) {
...
function showVictory(kind) {
    ...
    if (exerciseDone) {
        ...
        openModal({
            title: "¡ENHORABUENA! Has completado este ejercicio 🎉 " + "⭐".repeat(stars),
            text: "Has completado todas las etapas de este ejercicio.",
            primaryText: "Continuar",
            secondaryText: "Repetir última etapa",
            tertiaryText: "Repetir ejercicio",
    ...
    stageState[kind].stars = stars;
    openModal({
        title: "Etapa " + stageNumber + " de 3 completada  " + "⭐".repeat(stars),
        text: "¡Muy bien! Puedes continuar o repetir esta etapa.",
        primaryText: "Continuar",
        secondaryText: "Repetir etapa",
        tertiaryText: "",
...
function showFail(kind) {
    ...
    flashScreen("red");
    showToast(`¡Ups! Error 😅 Penalización: -${rollback}`);
    updateGateUI();
...
function updateGateUI() {
    ...
    if (currentExerciseStep === 0) {
        ...
        btnTopPanelEl.textContent = "Mostrar panel";
    } else {
        topPanelEl.style.display = topPanelVisibleInPractice ? "block" : "none";
        btnTopPanelEl.style.display = "inline-flex";
        btnTopPanelEl.textContent = topPanelVisibleInPractice ? "Ocultar panel" : "Mostrar panel";
    }
    if (!kind) {
    ...
    exerciseStepEl.textContent =
        `Etapa ${currentExerciseStep} de 3 • ❤️ ${s.lives} • ✅ ${s.correct}/${cfg.needCorrect} • 🎯 ${acc}% (mín. ${needAcc}%) • XP ${gameXP}`;
}
...
function updateStatsView() {
    ...
    statsEl.textContent =
        `Estadística de este ejercicio (solo esta sesión): elegir — ${st.mc.correct}/${st.mc.total}, escribir — ${st.write.correct}/${st.write.total}, oraciones — ${st.sent.correct}/${st.sent.total}.`;
}
...
function updateExplainVisibility() {
    if (currentExerciseStep === 0) {
        ...
        btnToggleExplain.textContent = "Ocultar explicación";
    } else {
        btnToggleExplain.disabled = false;
        explainEl.style.display = explainVisible ? "block" : "none";
        btnToggleExplain.textContent = explainVisible ? "Ocultar explicación" : "Mostrar explicación";
    }
}
...
function updateExerciseVisibility() {
    ...
    if (currentExerciseStep === 0) {
        exerciseStepEl.textContent = "Lee la explicación con calma y luego pulsa «Empezar ejercicios».";
        btnNextEx.textContent = "Empezar ejercicios";
    } else if (currentExerciseStep === 1) {
        exerciseStepEl.textContent = "Etapa 1 de 3: elige la frase correcta.";
        btnNextEx.textContent = "Ir a la etapa 2";
    } else if (currentExerciseStep === 2) {
        exerciseStepEl.textContent = "Etapa 2 de 3: escribe la frase.";
        btnNextEx.textContent = "Ir a la etapa 3";
    } else {
        exerciseStepEl.textContent = "Etapa 3 de 3: traduce las oraciones.";
        btnNextEx.textContent = "Etapas completadas — puedes ir al siguiente ejercicio";
    }
...
function nextExerciseStep() {
    ...
    if (currentExerciseStep < 3) {
        ...
        openModal({ title: "Última etapa", text: "Este ejercicio ya está en la última etapa. Puedes repetirlo o continuar.", primaryText: "Vale" });
    }
}
...
function loadBlock(index) {
    ...
    blockTitleEl.textContent = `Ejercicio ${currentBlockIndex + 1}. ${block.title}`;
    blockProgressTextEl.textContent = `Ejercicio ${currentBlockIndex + 1} de ${total}`;
...
function loadMCQuestion() {
    ...
    if (!block || !block.vocab || !block.vocab.length) {
        mcQuestionEl.textContent = "No hay palabras para este ejercicio.";
...
function checkMCAnswer(button, chosen, correct) {
    ...
    if (normalizeAnswer(chosen) === normalizeAnswer(correct)) {
        flashScreen("green");
        mcFeedbackEl.textContent = "✅ ¡Correcto!";
        mcFeedbackEl.className = "feedback correct";
        st.correct++;
        onCorrect("mc");
        speakRussian(correct);
        ...
        if (wrongAttemptsMC >= 3) {
            mcFeedbackEl.textContent = "❌ No del todo. Pista: la respuesta correcta es " + correct;
            speakRussian(correct);
        } else {
            mcFeedbackEl.textContent = "❌ No del todo. Intenta de nuevo.";
        }
...
function loadWQuestion() {
    ...
    const block = blocks[currentBlockIndex];
    if (!block || !block.vocab || !block.vocab.length) {
        wQuestionEl.textContent = "No hay palabras para este ejercicio.";
        return;
    }
...
function checkWAnswer() {
    ...
    if (normalizeAnswer(user) === normalizeAnswer(card.en)) {
        flashScreen("green");
        wFeedbackEl.textContent = "✅ ¡Muy bien! Así se escribe.";
        wFeedbackEl.className = "feedback correct";
        st.correct++;
        onCorrect("write");
        speakRussian(card.en);
        updateStatsView();
        ...
        if (wrongAttemptsWrite >= 3) {
            wFeedbackEl.textContent = "❌ Casi. Pista: la respuesta correcta es " + card.en;
            speakRussian(card.en);
        } else {
            wFeedbackEl.textContent = "❌ Casi. Corrige y pulsa de nuevo.";
        }
...
function showWAnswer() {
    ...
    if (!block || !block.vocab || !block.vocab.length) return;
    const card = block.vocab[currentVocabIndexWrite];
    wFeedbackEl.textContent = "Respuesta correcta: " + card.en;
    wFeedbackEl.className = "feedback";
    speakRussian(card.en);
}
...
function loadSentence() {
    ...
    if (!block || !block.sentences || !block.sentences.length) {
        sQuestionEl.textContent = "No hay oraciones para este ejercicio.";
        sInputEl.value = "";
        sFeedbackEl.textContent = "";
        sFeedbackEl.className = "feedback";
        return;
...
function checkSentence() {
    ...
    if (normalizeAnswer(user) === normalizeAnswer(card.en)) {
        flashScreen("green");
        sFeedbackEl.textContent = "✅ ¡Muy bien! La oración está traducida correctamente.";
        ...
        if (wrongAttemptsSent >= 3) {
            sFeedbackEl.textContent = "❌ Casi. Pista: la opción correcta es:\n" + card.en;
        } else {
            sFeedbackEl.textContent = "❌ Casi. Intenta de nuevo o pulsa «Mostrar respuesta».";
        }
...
function showSentenceAnswer() {
    ...
    if (!block || !block.sentences || !block.sentences.length) return;
    const card = block.sentences[currentSentenceIndex];
    sFeedbackEl.textContent = "Opción correcta:\n" + card.en;
    sFeedbackEl.className = "feedback";
    speakRussian(card.en);
}
