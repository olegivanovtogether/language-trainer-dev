...
(function () {
    ...
    function saveProgress() {
        try {
            ...
            const p = {
                ...
                sentQueue: Array.isArray(sentQueue) ? sentQueue.slice() : [],
    ...
    function getResumePreview(progress) {
        ...
        if (step === 2 && block.sentences && block.sentences.length) {
            let pos = Math.max(0, Math.min(progress.sentPos || 0, block.sentences.length - 1));
            if (progress.sentQueue && progress.sentQueue.length && progress.sentQueuePos != null && progress.sentQueue[progress.sentQueuePos] != null) {
                pos = Math.max(0, Math.min(progress.sentQueue[progress.sentQueuePos], block.sentences.length - 1));
            }
    ...
    function showResumeModal(progress, handlers) {
        ...
        } else if (step === 2) k = (progress.sentQueuePos != null ? progress.sentQueuePos : (progress.sentPos || 0)) + 1;
    ...
    function applyResumeState(progress) {
        ...
        sentQueue = Array.isArray(progress.sentQueue) ? progress.sentQueue.slice() : [];
        sentQueuePos = Math.max(0, (progress.sentQueuePos != null ? progress.sentQueuePos : 0));
        ...
        const seqSent = progress.seqSent !== undefined ? progress.seqSent : (progress.sentQueuePos != null ? progress.sentQueuePos : (progress.sentPos != null ? progress.sentPos : 0));
        seq.mc.pos = Math.max(0, Math.min(seqMC, nV));
        seq.mc.last = -1;
        seq.write.pos = Math.max(0, Math.min(seqWrite, nV));
        seq.write.last = -1;
        seq.sent.pos = Math.max(0, Math.min(seqSent, getFinalStageTargetCount()));
        seq.sent.last = -1;
        if (nV > 0) {
            GAME.stage.mc.needCorrect = nV * 4;
            GAME.stage.write.needCorrect = nV * 4;
        }
        if (nS > 0) GAME.stage.sent.needCorrect = getFinalStageTargetCount();
        while (writeMistakeCounts.length < nV) writeMistakeCounts.push(0);
        if (writeMistakeCounts.length > nV) writeMistakeCounts = writeMistakeCounts.slice(0, nV);
        while (mcMistakeCounts.length < nV) mcMistakeCounts.push(0);
        if (mcMistakeCounts.length > nV) mcMistakeCounts = mcMistakeCounts.slice(0, nV);
    ...
    function getFinalStageTargetCountByTotal(total) {
        if (total <= 0) return 0;
        if (total === 1) return 1;
        return Math.max(5, Math.ceil(total / 3));
    }
    function buildFinalStageQueue(total) {
        if (total <= 0) return [];
        if (total === 1) return [0];
        const target = getFinalStageTargetCountByTotal(total);
        const source = [];
        for (let i = 0; i < total; i++) source.push(i);
        if (total >= 5) {
            const uniqueCount = Math.min(total, target);
            const shuffled = source.slice();
            shuffle(shuffled);
            return shuffled.slice(0, uniqueCount);
        }
        // 2..4 sentences: repeat shuffled cycles until reaching 5 cards.
        const out = [];
        while (out.length < target) {
            const cycle = source.slice();
            shuffle(cycle);
            for (let i = 0; i < cycle.length && out.length < target; i++) out.push(cycle[i]);
        }
        return out;
    }
    function getFinalStageTargetCount() {
        const total = getNFor("sent");
        if (total <= 0) return 0;
        if (Array.isArray(sentQueue) && sentQueue.length) return sentQueue.length;
        return getFinalStageTargetCountByTotal(total);
    }
    function ensureFinalStageQueue(forceRebuild) {
        const total = getNFor("sent");
        if (total <= 0) {
            sentQueue = [];
            sentQueuePos = 0;
        ...
        const hasValidQueue = Array.isArray(sentQueue) && sentQueue.length && sentQueue.every(function (idx) {
            return typeof idx === "number" && idx >= 0 && idx < total;
        });
        if (forceRebuild || !hasValidQueue) {
            sentQueue = buildFinalStageQueue(total);
            sentQueuePos = 0;
        }
        if (sentQueuePos < 0) sentQueuePos = 0;
        if (sentQueuePos >= sentQueue.length) sentQueuePos = Math.max(0, sentQueue.length - 1);
        GAME.stage.sent.needCorrect = Math.max(1, sentQueue.length);
    }
    ...
    function showFail(kind) {
        ...
        if (isCombinedStage && !remediationActive) {
            ...
            sentQueuePos = Math.max(0, sentQueuePos - rollback);
            if (seq && seq.sent) {
                seq.sent.pos = sentQueuePos;
                seq.sent.last = -1;
            }
        } else {
            try {
    ...
    function onCorrect(kind) {
        ...
        s.streak++;
        gameXP += 10;
        if (kind === "sent" && currentExerciseStep === 2) {
            sentQueuePos++;
            if (seq && seq.sent) seq.sent.pos = sentQueuePos;
        }
        if (kind === "mc" || kind === "write") {
            if (currentExerciseStep === 1) {
            ...
            if (currentExerciseStep !== 1 && correctSinceRollback[kind] >= rollbackSteps) {
                const progressPct = Math.round(getProgressPercentMcWrite(kind) * 100);
                const milestoneKey = kind;
                if (progressPct >= 75 && lastEncouragementMilestone[milestoneKey] < 75) {
                    ...
                    flashProgressBarGreen(function () {
                        ...
                        openModal({ title: "", text: pickRandomMessage(getMotivationalMessagesByProgress(progressPct)), primaryText: ui.ok || "Ок", onPrimary: closeModal });
                    });
                    return;
    ...
    function getMotivationalMessagesByProgress(progressPct) {
        const normalized = Math.max(0, Math.min(100, progressPct || 0));
        const cfg = ui.motivationalMessages || {};
        let list = [];
        if (normalized < 25) list = cfg.upTo25 || [];
        else if (normalized < 50) list = cfg.upTo50 || [];
        else if (normalized < 75) list = cfg.upTo75 || [];
        else list = cfg.upTo100 || [];
        if (Array.isArray(list) && list.length) return list;
        if (normalized < 50) return [ui.encourageUnder50 || "Чудовий старт!"];
        if (normalized < 75) return [ui.encourageHalf || "Ти вже пройшов більше половини."];
        return [ui.encourageNearEnd || "Ще трохи і фініш!"];
    }
    function getCombinedTransitionModalText(fromPart, toPart) {
        const progressPct = Math.round(getProgressPercentCombined() * 100);
        const motivation = pickRandomMessage(getMotivationalMessagesByProgress(progressPct));
        const harderText = ui.beforeWritePartText || ui.encourageHarderSuffix || "А тепер трохи складніше.";
        if (fromPart === "mc" && toPart === "write") {
            if (motivation) return motivation + "\n\n" + harderText;
            return harderText;
        }
        if (motivation) return motivation;
        return ui.encourageUnder50 || "Рухаємось далі.";
    }
    ...
    function loadBlock(index) {
        ...
        try {
            ...
            if (nS > 0) GAME.stage.sent.needCorrect = getFinalStageTargetCountByTotal(nS);
        } catch (e) { }
        blockTitleEl.textContent = (ui.blockTitlePrefix || "Вправа ") + (currentBlockIndex + 1) + (ui.blockTitleSuffix || ". ") + block.title;
        blockProgressTextEl.textContent = (ui.blockTitlePrefix || "Вправа ") + (currentBlockIndex + 1) + (ui.blockProgressOf || " з ") + total;
        explainEl.innerHTML = (block.topicTag ? "<div class=\"topic-tag\">" + block.topicTag + "</div>" : "") + (block.explanation || "");
    ...
    function loadMCQuestion() {
        ...
        if (currentExerciseStep === 1 && combinedStagePart === "mc") {
            ...
            if (idx < 0) {
                const text = getCombinedTransitionModalText("mc", "write");
                openModal({
        ...
        if (currentExerciseStep === 1 && combinedStagePart === "write") {
            ...
            if (idx < 0) {
                ...
                if (currentBatchStart >= n) {
                    ...
                    openModal({
                        title: ui.encourageTitle || "",
                        text: getCombinedTransitionModalText("write", "mc"),
                        primaryText: ui.continueLabel || "Продовжити",
                        secondaryText: "",
                        tertiaryText: "",
                        showCloseX: false,
    ...
    function loadSentence() {
        ...
        if (sentQueuePos < 0) sentQueuePos = 0;
        if (sentQueuePos >= sentQueue.length) {
            if (stageGoalMet("sent")) return;
            sentQueuePos = Math.max(0, sentQueue.length - 1);
        }
        currentSentenceIndex = sentQueue[sentQueuePos];
        if (seq && seq.sent) seq.sent.pos = sentQueuePos;
    ...
    sNextBtn.addEventListener("click", function () {
        if (currentExerciseStep === 2) {
            ensureFinalStageQueue(false);
            if (sentQueuePos < sentQueue.length - 1) sentQueuePos++;
            if (seq && seq.sent) seq.sent.pos = sentQueuePos;
        }
        loadSentence();
    });
